...
定期复盘节奏。每次运行**至多一次 LLM 调用**，只读账本、只写复盘报告与一条低置信度参考记忆，接触不到提案与下单路径。
...
下限用于防止误配把复盘变成第二条决策回路。两个周期同时到期时**大周期优先**，小周期顺延到下一 tick（两次运行之间至少间隔 2 分钟）。进程重启后从库中该周期最新报告的时间戳恢复游标：停机数日只补跑**一次**，不会逐槽回放。
